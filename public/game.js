...
class players {
  ...
  playerUpdate() {
    //synka med andra spelare eller något??
    socket.on("gameStateUpdate", (state) => {
      // Update players2
      for (const playerId in state.players2) {
        if (playerId === socket.id) {
          // This is the current player, so we might not need to update them
          // (unless the server is correcting their position)
        } else {
          // Update other players
          if (!players2[playerId]) {
            // If this player is new, add them to the local game state
            players2[playerId] = new Player(state.players2[playerId].x, state.players2[playerId].y);
          } else {
            // Update existing players2' positions
            players2[playerId].pos[0] = state.players2[playerId].x;
            players2[playerId].pos[1] = state.players2[playerId].y;
          }
        }
      }

      // Update projectiles
      projectileList = state.projectiles.map((proj) => new Projectile(proj.pos, proj.dir, proj.speed));

      // Update monsters
      monsterList = state.monsters.map((mob) => new Monster(mob.pos, mob.speed, mob.width, mob.height, mob.img, mob.hp));

      // Update other game objects (e.g., power-ups, particles)
      // ...
    });
    const players2 = {};

    socket.on("playerConnected", (playerId) => {
      players2[playerId] = new players(); // Assuming you have a Player class
    });

    socket.on("playerDisconnected", (playerId) => {
      delete players2[playerId];
    });

    socket.on("playerMoved", (data) => {
      if (players2[data.id]) {
        players2[data.id].pos = [data.x, data.y];
      }
    });

    socket.on("playerShot", (data) => {
      // Handle other player's shooting
    });
    ...
    socket.emit("playerMove", { x: player.pos[0], y: player.pos[1] });
  }
...
function Update() {
  ...
  if (currentTime - lastFireTime > fireCooldown) {
    ...
    socket.emit("playerShoot", { dirX: shootDirX, dirY: shootDirY });
  }
  if (Daedalus != null) {
    bossmusic.play();
...
function Draw() {
  ...
  if (victory == false) {
    ...
    ctx.drawImage(healthBar2, canvas.width / 2 - 95 + (190 - 190 * (monstersLeft / 100)), 53, 190 * (monstersLeft / 100), 18);
  }
